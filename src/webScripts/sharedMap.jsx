...
function ChoosePin({greenPins, redPins, pinColorHook}){
    ...
    function GreenBtn(){
        if(pinColor == "green"){
            return <div className="p-2 bg-green-500 rounded-xl">
                Using Green <div className="font-semibold text-sm">{greenPins} remaining</div> 
            </div>
        }else{
            return <div onClick={()=>{pinColorSelect("green")}} className="p-2 border-green-500 border-2 hover:bg-green-300 rounded-xl">
                Green <div className="font-semibold text-sm">{greenPins} remaining</div>
            </div>
        }
    }
    ...
    function RedBtn(){
        if(pinColor == "red"){
            return <div className="p-2 bg-red-500 rounded-xl">
                Using Red <div className="font-semibold text-sm">{redPins} remaining</div>
            </div>
        }else{
            return <div onClick={()=>{pinColorSelect("red")}} className="p-2 border-red-500 hover:bg-red-300 border rounded-xl">
                Red <div className="font-semibold text-sm">{redPins} remaining</div>
            </div>
        }
    }
...
function DataPanel({config, userId}){
    ...
    return <div className="mx-5 m-2 border bg-slate-900 font-bold rounded-xl border-slate-900">
        <h2 className="text-stone-100 text-center uppercase animate-pulse">Live Infobox</h2>
        <div className="bg-stone-200 p-2 rounded-bl-xl rounded-br-xl">
            <div className="mb-2">
                <div className="uppercase text-sm text-slate-600/80">Question</div>
                <h2 className="text-xl">{curRoundCfg.question}</h2>
            </div>
            <div className="columns-4 gap-2">
                <div className="w-full">
                    <div className="uppercase text-sm text-slate-600/80">Round</div>
                ...
                <div className="w-full">
                    <div className="uppercase text-sm text-slate-600/80">State</div>
                    <h3>{config.state}</h3>
                </div>
                <div className="w-full">
                    <div className="uppercase text-sm text-slate-600/80">Earned</div>
...
function getConfig(){
    return fetch('/trust/shared/requestConfig').then(
...
function selectColor(color){
    ...
    sharedMapRoot.render(
        <SharedMap 
            pinHandlers={mapHandlers} 
            markers={markers} 
            currentColor={pinColor}></SharedMap>
    )
}

function placePin(lat, lon, color, userId){
    liveSocket.emit('pinPlaced', {'lat':lat, 'lon':lon, 'color': color, 'userId':userId});
}
...
function movePin(pinId, userId, lat, lon){
    liveSocket.emit('pinMoved', {'userId': userId, 'pinId': pinId, 'lat': lat, 'lon':lon})
}
...
function updateView(){
    if(latestLiveData != undefined){
        if(latestLiveData.state == "running"){
            ...
            sharedMapRoot.render(
                <ReviewMap pins={latestLiveData.curPins} crimesJson={latestLiveData.reviewCrimes}/>
            );
        ...
        if(latestLiveData.state != "review"){
            userPanelRoot.render(
                Object.values(latestLiveData.users).map((user) => {
                    return <UserPanel key={user.userId+"_panel"} {...user}></UserPanel>
                })
            )
        }
